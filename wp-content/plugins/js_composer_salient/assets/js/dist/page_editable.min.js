window.vc_iframe = {
    ...
    window.vc_iframe.showNoContent = function(e) {
       ...
       //vc col mobile fixes
       // legacy col system
       if ( jQuery('body[data-flex-cols="true"]').length === 0 ) {
            jQuery('.wpb_row').each(function() {
                var $row = jQuery(this);
                var $span = $row.find('.span_12');
                if (!$span.length) return;

                // Measure container once (viewport-based is fine since we compare like-to-like)
                var spanLeft = $span[0].getBoundingClientRect().left;

                var cols = $span[0].querySelectorAll('[class*="vc_col-"]');

                // 1) Measure (read-only)
                var isNoLeft = new Array(cols.length);
                for (var i = 0; i < cols.length; i++) {
                    var colLeft = cols[i].getBoundingClientRect().left;
                    isNoLeft[i] = colLeft < (spanLeft + 27);
                }

                // 2) Write (class changes)
                for (var j = 0; j < cols.length; j++) {
                    cols[j].classList.toggle('no-left-margin', isNoLeft[j]);
                }
            });
        }
       /*nectar addition end */
    ...
    window.vc_iframe.setSortable = function(e) {
       ...
       //nectar addition end
        ...
        v.sortable({
            ...
            items: "> [data-tag=vc_row], > [data-tag=vc_section], > [data-tag=vc_grid_container], > [data-tag=vc_flexbox_container]",
            //nectar addition end
            cursor: "move",
            cursorAt: {
