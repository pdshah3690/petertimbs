...
if( 'post' === $post_type ) {
	...
	/**
	 * Build term objects from the saved term IDs directly.
	 *
	 * Rationale:
	 * - `custom_query_tax` is stored as term IDs (e.g. "78" or "78,79").
	 * - Using `get_terms()` across all public taxonomies can return empty in some setups
	 *   (e.g. language plugins filtering terms), which would result in no tax_query and thus ALL posts.
	 */
	$term_ids = wp_parse_id_list( $custom_query_tax );
	$terms    = array();
	 ...
	 * If `get_term()` cannot resolve the IDs (e.g. ambiguous term IDs across taxonomies),
	 * fall back to the original `get_terms()` approach to avoid regressions.
	 */
	if ( empty( $terms ) && ! empty( $custom_query_tax ) ) {
