...
if( $link_style === 'play_button_2' || $link_style === 'play_button_mouse_follow' ) {
	...
	$link_screen_reader_text = ( !empty($link_screen_reader) ) ? '<span class="screen-reader-text">' . esc_html($link_screen_reader) . '</span>' : '<span class="screen-reader-text">' . esc_html__('Play Video','salient-core') . '</span>';
	...
	echo '<div class="nectar-video-box" data-color="'.esc_attr(strtolower($nectar_play_button_color)).'" '.$mouse_markup.' data-play-button-size="'.esc_attr($play_button_size).'" data-border-radius="'.esc_attr($border_radius).'" data-hover="'.esc_attr($hover_effect).'" data-shadow="'.esc_attr($box_shadow).'"><div class="inner-wrap"><a href="'.esc_url($video_url).'" '.$no_follow_attr.'class="full-link pp">'.$link_screen_reader_text.'</a>'. $image;
}
