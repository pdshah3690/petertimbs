...
if( 'vertical_scrolling' !== $style ) {
	...
	foreach ( $tab_titles as $tab ) {
	    ...
	    if(isset($tab_matches[1][0])) {
			...
			// Generate clean tab anchor - strip percent-encoded chars from non-Latin titles.
			$tab_anchor = preg_replace('/%[0-9a-fA-F]{2}/', '', sanitize_title( $tab_matches[1][0] ));
			$tab_anchor = preg_replace('/-+/', '-', $tab_anchor);
			$tab_anchor = trim($tab_anchor, '-');

			// Fall back to shortcode id for fully non-Latin titles.
			if( empty($tab_anchor) ) {
				preg_match('/\sid="([^\"]+)"/i', $tab[0], $tab_id_matches, PREG_OFFSET_CAPTURE );
				if( isset($tab_id_matches[1][0]) ) {
					$tab_anchor = $tab_id_matches[1][0];
				}
			}

	        $tabs_nav .= '<li class="'.implode(' ',$tabbed_li_classes).'"><a role="button" href="#tab-'. esc_attr($tab_anchor) .'" '.$active_class.'><span>' . wp_kses_post($tab_matches[1][0]) . '</span></a></li>';
		...
		if( $style === 'toggle_button' && $tab_index === 1) {
			$tabs_nav .= '<li class="toggle-button"><span role="button" class="toggle-button-inner nectar-color-'.strtolower($tab_color).' nectar-bg-'.strtolower($tab_color).'"><span class="screen-reader-text">'.__('Toggle', 'salient-core').'</span><span class="circle"></span></span></li>';
		}
