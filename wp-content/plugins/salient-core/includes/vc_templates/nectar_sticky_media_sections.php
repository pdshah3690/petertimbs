...
$css_parts = [];
if( $effect === 'scale_fade' ) {
    $css_parts[] = '--scale-fade-color:' . $scale_fade_color;
}
$section_count = substr_count($content, '[nectar_sticky_media_section');

$css_parts[] = '--section-count:' . $section_count;

$GLOBALS['nectar-sticky-media-section-count'] = 0;
$GLOBALS['nectar-sticky-media-type'] = $type;
$style_attr = '';
if ( ! empty( $css_parts ) ) {
    $style_attr = ' style="'.implode(';', $css_parts).'"';
}
echo '<div class="'.nectar_clean_classnames(implode(' ',$el_classes)).'"'.$style_attr.'>';
if ( $type === 'scroll-pinned-sections' || $type === 'horizontal-scrolling' || $type === 'layered-card-reveal' ) {
    $navigation_markup = '';
    if ( 'yes' === $navigation ) {
