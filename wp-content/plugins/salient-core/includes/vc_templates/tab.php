...
$tab_anchor = preg_replace('/%[0-9a-fA-F]{2}/', '', sanitize_title( $title ));
$tab_anchor = preg_replace('/-+/', '-', $tab_anchor);
$tab_anchor = trim($tab_anchor, '-');

// Fall back to shortcode id for fully non-Latin titles.
if( empty($tab_anchor) && isset($atts['id']) ) {
	$tab_anchor = $atts['id'];
}

$output .= "\n\t\t\t" . '<div id="tab-'. esc_attr($tab_anchor) .'" '.$icon_attr .' class="'.esc_attr($css_class).'">' . $inner_wrap . $icon_markup;
