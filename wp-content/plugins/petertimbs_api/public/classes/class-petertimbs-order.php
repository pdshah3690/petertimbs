...
class Petertimbs_Order {
	...
	public function create($request) {
        ...
        if(empty($post['stripe_cc_token_key']) && $post['payment_method'] == 'stripe') {
			$response['success'] = false;
			$response['message'] = "Provide valid stripe payment method";
		}
		...
		if(empty($response)) {
            ...
            if($post['order_type'] == 'delivery') {
				...
				} else if(in_array($post['billing_postcode'],$zone_two)) {
				...
				} else if(in_array($post['billing_postcode'],$zone_three)) {
				...
				} else if(in_array($post['billing_postcode'],$zone_four)) {
				...
				} else if(in_array($post['billing_postcode'],$north_island_postcodes) && $is_merchandise == true) {
				...
				} else if(in_array($post['billing_postcode'],$south_island_postcodes) && $is_merchandise == true) {
            ...
            if($post['payment_method'] == 'stripe') {
                $_POST['stripe_cc_token_key'] = $post['stripe_cc_token_key'];
                $_POST['stripe_cc_payment_type_key'] = "new";
        		WC()->payment_gateways();
    			$available_gateways = WC()->payment_gateways()->get_available_payment_gateways();
    			$gateway_id = 'stripe_cc';
	    		$gateway = $available_gateways[$gateway_id];
            	$payment_result = $gateway->payment_object->process_payment( $order );
                if($payment_result->complete_payment == 1) {
    			    $order->set_payment_method($gateway_id);
    				$order->set_payment_method_title($gateway->get_title());
    				update_post_meta($order->get_id(), '_transaction_id', $payment_result->charge->id);
                    update_post_meta($order->get_id(), "_stripe_currency", "nzd");
                    update_post_meta($order->get_id(), "_wc_stripe_charge_status", "succeeded");
