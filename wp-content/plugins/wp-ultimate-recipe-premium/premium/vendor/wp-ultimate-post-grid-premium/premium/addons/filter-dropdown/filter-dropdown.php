...
class WPUPG_Filter_Dropdown extends WPUPG_Premium_Addon {
    ...
    public function grid_cache_filter( $filter, $cache, $grid ) {
        if( $grid->filter_type() == 'dropdown' || $grid->filter_type() == 'text_dropdown' ) {
            ...
            foreach( $taxonomies as $taxonomy_key ) {
                if( isset( $cache['taxonomies'][$taxonomy_key] ) ) {
                    ...
                    if( $taxonomy ) {
                        ...
                        $select_options = apply_filters( 'wpupg_grid_cache_filter_dropdown_options', $select_options, $grid, $taxonomy_key );
            ...
            if( $grid->filter_type() == 'text_dropdown' && WPUltimatePostGrid::is_addon_active('filter-text') ) {
    ...
    public function filter_shortcode( $output, $grid) {
        if( $grid->filter_type() == 'dropdown' || $grid->filter_type() == 'text_dropdown' ) {
            ...
            $output .= '<div id="wpupg-grid-' . $grid->slug() . '-filter" class="wpupg-filter wpupg-filter-' . $grid->filter_type() . '" data-grid="' . $grid->slug() . '" data-type="'. $grid->filter_type() . '" data-multiselect-type="' . $grid->filter_multiselect_type() . '" data-inverse="' . $inverse . '">';
            $output .= $grid->filter();
            $output .= '</div>';
        }
