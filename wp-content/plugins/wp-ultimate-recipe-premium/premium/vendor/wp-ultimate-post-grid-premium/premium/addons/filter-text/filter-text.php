    ...
    public function ajax_filter_text()
    {
        if( check_ajax_referer( 'wpupg_grid', 'security', false ) )
        {
            ...
            if( !is_null( $post ) ) {
                ...
                echo $grid->draw_posts( -2, $post_ids );
            }
        }
        die();
    }

    public function get_filter( $grid ) {
        $filter_style = $grid->filter_style();
        $placeholder = $filter_style['text']['placeholder_text'];

        return '<input type="text" class="wpupg-filter-text-input" placeholder="' . $placeholder . '">';
    }

    public function grid_cache_filter( $filter, $cache, $grid ) {
        if( $grid->filter_type() == 'text' ) {
            $filter .= $this->get_filter( $grid );
        }
