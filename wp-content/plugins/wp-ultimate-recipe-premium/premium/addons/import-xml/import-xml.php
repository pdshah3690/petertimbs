...
class WPURP_Import_Xml extends WPURP_Premium_Addon {
    ...
    public function import_manual_page() {
        ...
        foreach( $recipes as $recipe ) {
            $this->import_xml_recipe( $recipe, $i, $post_status );
            $i++;
        }
    ...
    public function import_xml_recipe( $xml_recipe, $recipe_number, $post_status = 'draft' ) {
        ...
        foreach( $xml_recipe->instructions as $instruction_group ) {
            ...
            foreach( $instruction_group->instruction as $instruction ) {
                $new_instruction = array(
                    'description' => (string) $instruction,
                    'image' => '',
                    'group' => $group,
                );

                if( isset( $instruction->attributes()->imageUrl ) ) {
                    $instruction_image_url = (string) $instruction->attributes()->imageUrl;
                    $instruction_image_id = $this->get_or_upload_attachment( $post_id, $instruction_image_url );

                    if( $instruction_image_id ) {
                        $new_instruction['image'] = $instruction_image_id;
                    }
                }
        ...
        $post_id = intval( isset ( $xml_recipe->postId ) ? (string) $xml_recipe->postId : '' );
        if( $post_id == 0 || FALSE === get_post_status( $post_id ) ) {
