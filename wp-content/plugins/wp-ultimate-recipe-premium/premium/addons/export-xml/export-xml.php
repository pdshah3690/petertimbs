...
class WPURP_Export_Xml extends WPURP_Premium_Addon {
    ...
    public function export_xml_recipe( $recipe_id ) {
        ...
        if( WPUltimateRecipe::option( 'export_recipes_xml_post_id', '0' ) == '1' ) {
            $xml[] = array(
                'name' => 'postId',
                'value' => $recipe->ID(),
            );
        ...
        if( $recipe->is_present( 'recipe_instructions' ) ) {
            ...
            foreach( $recipe->instructions() as $instruction ) {
                ...
                if( $instruction['image'] ) {
                    ...
                    $group_instruction['attributes'] = array(
                        'imageUrl' => $full_img_url,
                    );
                }

                $group_instructions[] = $group_instruction;
            }
