...
if( !function_exists('nectar_portfolio_processing') ) {
									...
									<div class="work-item style-5" data-custom-content="<?php echo esc_attr( $using_custom_content ); ?>" data-text-align="<?php echo esc_attr( $masonry_item_content_pos ); ?>">
										...
										//no parallax images set
											...
											if (!empty($custom_thumbnail)) {
												...
												$thumb_url_src = isset($thumbnail_url[0]) ? $thumbnail_url[0] : '';

												if($bypass_image_cropping === 'true') {
													echo '<img class="sizer '.esc_attr($skip_lazy_class_name).'" src="'. esc_url( $thumb_url_src ) .'" alt="'.get_the_title().'" />';
												} else {
												...
												echo '<div class="parallaxImg-layer" data-img="'. esc_url( $thumb_url_src ) .'"></div>';
	...
	if( !function_exists('nectar_recent_projects') ) {
			...
			extract(shortcode_atts(array(
				...
				if(!empty($heading)) {
					if($full_width_carousel === 'true'){
						...
						if(!empty($page_link_text)) {
							$title_label_output .= '<a href="'. esc_url($page_link_url).'" class="button"> / '. wp_kses_post($page_link_text) .'</a>';
						}
				...
				if($title_labels === 'true') {
					...
					if(!empty($recent_projects_link_text) && strlen($recent_projects_link_text) > 2) {
						$title_label_output .= '<a href="'. esc_url($portfolio_link).'" class="button"> / '. wp_kses_post($recent_projects_link_text) .'</a>';
					}
					$title_label_output .= '</h2>';
				}
		...
		function nectar_project_categories_render($atts, $content = null) {
			...
			// Determine taxonomy slug from attribute. Default to project-type when empty.
			$taxonomy_slug = 'project-type';
			if ( ! empty( $taxonomy ) ) {
				if ( 'project-categories' === $taxonomy ) {
					$taxonomy_slug = 'project-type';
				} elseif ( 'project-attributes' === $taxonomy ) {
					$taxonomy_slug = 'project-attributes';
				} else {
					$taxonomy_slug = $taxonomy;
				}
			}

			$project_categories = get_the_terms($post->ID, $taxonomy_slug);
			  ...
			  foreach ( $project_categories as $term ) {
				...
				if( isset($term->slug) ) {
					if ( $link_categories === 'true') {
						$output .= '<a class="' . esc_attr( $term->slug ) .'" href="' . esc_url( get_term_link( $term ) ) . '">' . esc_html( $term->name ) . '</a>';
					} else {
			...
			if ($category_markup) {
				...
				// Direction.
				$dir = (in_array($direction, array('horizontal','vertical'), true)) ? $direction : 'horizontal';
				$classnames[] = 'direction-' . $dir;
				...
				// Gap CSS variable.
				$style_attr = '';
				if ( isset( $gap ) && $gap !== '' && $gap !== '10' ) {
					$style_attr = ' style="--term-gap: ' . esc_attr( $gap ) . 'px;"';
				}
				return '<span class="'.implode(' ', $classnames) .'"'.$style_attr.'>'.$category_markup.'</span>';
			}
