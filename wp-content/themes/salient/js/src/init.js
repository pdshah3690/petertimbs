...
(function($, window, document) {
    ...
    function clamp(val,min, max) {
		...
		*/
		...
		function nectarScriptLoadedCallback( timeout, callback, loadCondition ) {
			var pollTimer;
			var eventListeners = [];
			var pollInterval = 100;
			var pollLimit = 30; // 3s fallback.
			var pollCount = 0;
			...
			['load', 'salient-delayed-js-loaded'].forEach(function(eventName) {
				...
				waypoint    = new Waypoint({
					...
					handler: function () {
						...
						if($endNum.toString().indexOf('.') !== -1) {
							...
							if( userDecimals > 2 && userDecimals < 6) {
								decimalPlaces = userDecimals;
							}

							if ($that.is('[data-single-decimal-place]') && $that.attr('data-single-decimal-place') == 'true') {
								decimalPlaces = 1;
							}
		...
		* @since 3.0
		*/
		function handleTabDeepLinkFromHref(linkHref) {

			if (!linkHref || linkHref.indexOf('?') === -1 || /[?&]tab=/.test(linkHref) === false) {
				return;
			}

			var tabQueryIndex = linkHref.indexOf('?');
			var hashIndex = linkHref.indexOf('#');
			var queryEndIndex = (hashIndex > tabQueryIndex) ? hashIndex : linkHref.length;
			var linkQuery = linkHref.substring(tabQueryIndex, queryEndIndex);
			var linkQueryParams = getQueryParams(linkQuery);
			var tabParam = (typeof linkQueryParams['tab'] !== 'undefined') ? linkQueryParams['tab'] : undefined;

			if (typeof tabParam === 'undefined') {
				return;
			}

			tabbbedDeepLinking(tabParam);
		...
		* @since 3.0
		*/
		function handleAccordionDeepLinkFromHref(linkHref) {

			if (!linkHref || linkHref.indexOf('?') === -1 || /[?&]toggle=/.test(linkHref) === false) {
				return;
			}

			var toggleQueryIndex = linkHref.indexOf('?');
			var hashIndex = linkHref.indexOf('#');
			var queryEndIndex = (hashIndex > toggleQueryIndex) ? hashIndex : linkHref.length;
			var linkQuery = linkHref.substring(toggleQueryIndex, queryEndIndex);
			var linkQueryParams = getQueryParams(linkQuery);
			var toggleParam = (typeof linkQueryParams['toggle'] !== 'undefined') ? linkQueryParams['toggle'] : undefined;
			var focusParam = (typeof linkQueryParams['focus'] !== 'undefined') ? linkQueryParams['focus'] : undefined;

			if (typeof toggleParam === 'undefined') {
				return;
			}

			accordionDeepLinking(toggleParam, focusParam);
		...
		* @since 3.0
		*/
		function tabbbedDeepLinking(tabParamOverride) {

			var tabParam = (typeof tabParamOverride !== 'undefined') ? tabParamOverride : nectarGetQueryParam['tab'];
	...
	VerticalScrollingTabs.prototype.lineAnimation = function () {
		...
		* @since 3.0
		*/
		function scrollToElementWithHeaderOffset($element, shouldScroll) {
			if (!shouldScroll) {
				return;
			}

			setTimeout(function() {
				var $scrollTopDistance = $element.offset().top - nectarDOMInfo.adminBarHeight;

				if (nectarDOMInfo.winW < 1000 && $('#header-outer[data-mobile-fixed="1"]').length > 0) {
					$scrollTopDistance = $element.offset().top - $headerOuterEl.outerHeight() - nectarDOMInfo.adminBarHeight;
				}

				if (nectarDOMInfo.winW >= 1000 && $('#header-outer[data-remove-fixed="0"]').length > 0) {
					$scrollTopDistance = $element.offset().top - $headerOuterEl.outerHeight() - nectarDOMInfo.adminBarHeight;
				}

				if (nectarDOMInfo.winW >= 1000) {
					$scrollTopDistance -= 30;
				}
		...
		* @since 3.0
		*/
		function accordionDeepLinking(toggleParamOverride, focusParamOverride) {

			var toggleParam = (typeof toggleParamOverride !== 'undefined') ? toggleParamOverride : nectarGetQueryParam['toggle'];
			var focusParam = (typeof focusParamOverride !== 'undefined') ? focusParamOverride : nectarGetQueryParam['focus'];
			var shouldFocus = (typeof focusParam !== 'undefined' && focusParam.toString().toLowerCase() === 'true');

			if (typeof toggleParam != 'undefined') {
				var toggleParamString = toggleParam.toString().trim();
				var isIndexLink = /^[0-9]+$/.test(toggleParamString);

				if (isIndexLink) {
					var toggleIndex = parseInt(toggleParamString, 10);
					var $targetToggle = $('.toggles .toggle').eq(toggleIndex - 1);

					if ($targetToggle.length > 0) {
						var $toggleTrigger = $targetToggle.find('> h3 a, > .toggle-title a').first();

						$toggleTrigger.trigger('click');
						scrollToElementWithHeaderOffset($targetToggle, shouldFocus);
					}
				...
				$('.toggles').each(function () {
					...
					$(this).find('.toggle').each(function () {
						...
						if ($currentText == $getText) {
							var $toggleTrigger = $that.find('> h3 a, > .toggle-title a').first();

							$toggleTrigger.trigger('click');
							scrollToElementWithHeaderOffset($that, shouldFocus);
						}
					});
				});
			}
				...
				*/
				function nectarTestimonialSliders() {
					...
					if (typeof NectarTestimonialSlider !== 'undefined') {
						...
						nectarScriptLoadedCallback(0, nectarTestimonialSliders, function(){
							return (typeof NectarTestimonialSlider !== 'undefined');
						});
					}
            ...
            $('#slide-out-widget-area .off-canvas-menu-container li').on('mouseleave',function() {
				...
				*/
				function waypoints() {
					...
					if (typeof Vivus !== 'undefined') {
						...
						nectarScriptLoadedCallback(100, function(){
							svgAnimations();
						}, function(){
				...
				*/
				...
				function navigationSearchInit() {
					...
					$body.on('click', '#search-btn a:not(.inactive), #header-outer .mobile-search, .nectar-search-open-trigger', function (e) {
				...
				function animatedAnchorLinks() {
					...
					$('.single-product [data-gallery-style="left_thumb_sticky"]').length > 0) {
						...
						$body.on('click', '#header-outer nav .sf-menu a:not([href="#slide-out-widget-area"]), #header-outer .mobile-header .sf-menu a, #footer-outer .nectar-button, #footer-outer .widget_nav_menu a, #footer-widgets .textwidget a, #mobile-menu li a, .nectar-scrolling-tabs:not(.navigation_func_active_link_only) .scrolling-tab-nav a, .nectar-global-section a:not(.wpb_tabs_nav a), .container-wrap a:not(.wpb_tabs_nav a):not(.navigation_func_active_link_only .scrolling-tab-nav a):not(.comment-wrap .navigation a):not(.woocommerce-checkout a):not(.um-woo-view-order):not(.magnific):not([data-fancybox]):not(.woocommerce-tabs .tabs a):not(.slider-prev):not(.slider-next):not(.testimonial-next-prev a):not(.page-numbers), .swiper-slide .button a, #slide-out-widget-area a, #slide-out-widget-area .inner div a', function (e) {
							...
							handleTabDeepLinkFromHref(linkHref);
							handleAccordionDeepLinkFromHref(linkHref);
							...
							if ($hash && $body.find($hash).length > 0 && $hash != '#top' && $hash != '#search-box' && $hash != '' && $(this).attr('href').indexOf(window.location.href.split("#")[0]) !== -1 ||
							$(this).is('[href^="#"]') && $hash != '' && $body.find($hash).length > 0 && $hash != '#top' && $hash != '#search-box') {
					...
					$('.portfolio-items:not(.carousel)').each(function(i) {
						...
						if (typeof SalientPortfolio !== 'undefined') {
							...
							nectarScriptLoadedCallback(100, function(){
								...
								return (typeof SalientPortfolio !== 'undefined');
							});
						}
					...
					$('.posts-container').each(function (i) {
						...
						if ($(this).parent().hasClass('masonry') && !$(this).parent().hasClass('auto_meta_overlaid_spaced')) {
							...
							if (typeof NectarMasonryBlog == 'undefined') {
								...
								var $that = $(this);
								nectarScriptLoadedCallback(0, function(){
									$nectarMasonryBlogs[i] = new NectarMasonryBlog($that, fullWidthSections, blogLoadIn);
								}, function(){
									return (typeof NectarMasonryBlog !== 'undefined');
								});
				...
				*/
				function nectarFullPageInit() {
					...
					if ($('#nectar_fullscreen_rows').length > 0 && $().fullpage) {
						...
						nectarScriptLoadedCallback(100, nectarFullPageInit, function(){
							return ($('#nectar_fullscreen_rows').length > 0 && typeof NectarFullScreenRows !== 'undefined' && typeof jQuery !== 'undefined' && typeof jQuery.fn.fullpage !== 'undefined');
						});
					}
