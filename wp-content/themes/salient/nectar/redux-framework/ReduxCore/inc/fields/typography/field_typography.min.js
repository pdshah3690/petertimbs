 ...
 (function( $ ) {
  ...
  redux.field_objects.typography.select = function( selector, skipCheck ) {
      ...
      if ( $( '#' + mainID ).hasClass( 'typography-initialized' ) ) {
          ...
          if ( isPreviewSize == '0' ) {
              var fsUnits = units; var $fsUnitsField = $( selector ).parents( '.redux-container-typography:first' ).find('.typography-font-size-units select'); if( $fsUnitsField.length > 0 && ($fsUnitsField.val() == 'em' || $fsUnitsField.val() == 'rem') ) { fsUnits = $fsUnitsField.val(); }
          ...
          var lhUnits = units; var $lhUnitsField = $( selector ).parents( '.redux-container-typography:first' ).find('.typography-line-height-units select'); if( $lhUnitsField.length > 0 && ($lhUnitsField.val() == 'em' || $lhUnitsField.val() == 'rem') ) { lhUnits = $lhUnitsField.val(); } $( '#' + mainID + ' .typography-preview' ).css( 'line-height', height + lhUnits );
