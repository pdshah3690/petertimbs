...
<?php 
	$type = get_post_meta($this->order->get_id(), "_order_type", 1);
	$delivery_date = get_post_meta($this->order->get_id(), "_order_delivery_date", 1);
	$delivery_date = date("F j, Y", strtotime($delivery_date));
	$pickup_date = get_post_meta($this->order->get_id(), "_order_preparation_time", 1);
	$pickup_date = date("F j, Y H:i", strtotime($pickup_date));
?>
	...
	<tr>
		...
		<td class="order-data">
			<table>
				...
				<?php else: ?>
				<tr class="pickup-date-time">
					<th><?php _e( 'Pick up Date/Time:', 'woocommerce-pdf-invoices-packing-slips' ); ?></th>
